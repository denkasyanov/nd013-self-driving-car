...
public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++)
    {
      ...
      AD<double> psides_slope0 = 0;
      for (int i = 1; i < coeffs.size(); ++i)
      {
        psides_slope0 += i * coeffs[i] * CppAD::pow(x0, i-1);
      }
      AD<double> psides0 = CppAD::atan(psides_slope0);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // clear trajectory arrays for drawing
  x_trajectory.clear();
  y_trajectory.clear();
  ...
  double max_angle_rad = 25. / 180. * M_PI;


  for (int i = delta_start; i < a_start; ++i)
  {
    vars_lowerbound[i] = -max_angle_rad;
    vars_upperbound[i] =  max_angle_rad;
  }
  ...
  for (int i = 0; i < N - 1; i++)
  {
    x_trajectory.push_back(solution.x[x_start + i + 1]);
    y_trajectory.push_back(solution.x[y_start + i + 1]);
  }
